...
def select_kbest(X, y, k): 
    """
    Purpose
        To return the top features selected by the SelectKBest function
...
def lars_mod(X, y, selectors, scores, fit_x_train=None, fit_y_train=None):
   ...
   Returns
      scores: dataFrame containing the scores, features, and parameters of the created models
      y: 
   """
...
def GLM_mod(X, y, selectors, scores, fit_x_train=None, fit_y_train=None):
   """
   Purpose
      to create, train, and score GLM regression models
...
def score_on_train(X_train, y_train): 
   ...
   #reset values of y_val just needed columns
...
def score_on_validate(train_scores, X_val, y_val, X_train, y_train): 
   """
   Purpose
      Test models on the validation subset

   Parameters
      X: dataframe containing X subset of features for the data subset
      y: dataframe with series containing the target variable
      selectors: list of different feature and degree combinations for use with models
      X_train: X_train subset of data to fit the model
      y_train: X_train subset of data to fit the model

   Returns
      model_descriptions: dataFrame containing the scores, and y_val subset to faciliate repeated testing
      y_val: the y_val input
   """
   #reset values of y_val just needed columns
   ...
   #if statement to check each model for naming convention and feed them into the appropriate modeling function
   #takes out the correct features and parameters 
...
def score_on_test(X_test, y_test, X_train, y_train):
   """
   Purpose
      Perform fitting of of the model on the train subset, and testing on the test subset

   Parameters
      X_test: dataframe containing X subset of features for the test subset
      y_test: dataframe with series containing the target variable
      selectors: list of different feature and degree combinations for use with models
      X_train: X_train subset of data to fit the model when needing to score the subsets
      y_train: X_train subset of data to fit the model when needing to score the test

   Returns
      test_score: dataFrame containing the scores, features, and parameters of our best performing model
   """
